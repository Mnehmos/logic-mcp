...
function startHttpServer(appInstance, port) {
    return new Promise((resolve, reject) => {
...
server.tool("execute_logic_operation", executeLogicOperationInputSchema.shape, // Use .shape for ZodRawShape
{
    ...
    try {
        ...
        if (!currentChainId) {
            isNewChain = true;
            currentChainId = (0, crypto_1.randomUUID)();
            await new Promise((resolve, reject) => {
                db.run("INSERT INTO logic_chains (chain_id, description, created_at) VALUES (?, ?, ?)", [currentChainId, `Chain started with ${params.operation.type} operation`, startTime], (err) => {
                    if (err) {
...
Please generate a well-formulated query, a series of questions, or a detailed plan to obtain the needed information.`;
    ...
    const dbOperationInternalId = await new Promise((resolve, reject) => {
        db.run(`INSERT INTO operations (operation_id, primitive_name, input_data, output_data, status, start_time, end_time, context)
